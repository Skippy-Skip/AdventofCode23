...
fs.readFile("day7/input 7.txt", "utf8", (err, data) => {
    ...
    function determineHandType(array) {
        ...
        jokerAmount = array.filter(x => x == "J").length
        uniqueCards = [...new Set(array)]; //See how many unique cards a hand has, in order to assign it's handType.

        switch (uniqueCards.length) {
            case 5: //If there are 5 unique cards.

                if (jokerAmount === 0) {  //If there is no joker, hand type = 'High card'.
                    handType = 'High card';
                    highCard.push(hand);
                } else if (jokerAmount === 1) { //If there is 1 joker, hand type = 'One Pair'.
                    handType = 'One Pair'; 
                    onePair.push(hand);
                };
            ...
            case 4: //If there are 4 unique cards, hand type = 'One pair'.
                ...
                if (jokerAmount === 0) { //If there is no joker, hand type = "One pair".
                    handType = 'One Pair';
                    onePair.push(hand);
                } else if (jokerAmount === 1 || jokerAmount === 2) { //If there is one or two jokers, hand type = 'Three of a kind'.
            ...
            case 3: //If there are 3 unique cards, hand type = 'Two pair' or 'Three of a kind'.
                ...
                for (y=0; y<3; y++) { //Loop over every card in the unique set.
                    ...
                    if (handInfo.filter(x => x === uniqueCards[y]).length === 3) { //If a card appears 3 times in the full hand set, hand type = 'Three of a kind'.
                        if (jokerAmount === 0) { //If there are no jokers, hand type = 'Three of a kind'.
                            handType = 'Three of a kind';
                            threeOfAKind.push(hand);
                            break;
                        } else if (jokerAmount === 1 || jokerAmount === 3) { //If there are 1 or 3 jokers, hand type = 'Four of a kind'.
                ...
                if (handType === 'Two pair') { 
                    ...
                    if (jokerAmount === 0) { //If there is no joker, hand type remains 'Two pairs'.
                        twoPairs.push(hand);
                    } else if (jokerAmount === 1) { //If there is 1 joker, hand type = 'Full house'.
                        handType = 'Full house';
                        fullHouse.push(hand);
                    } else if (jokerAmount ===2) { //If there are 2 jokers, hand type = 'Four of a kind'.
                        handType = 'Four of a kind';
                        fourOfaKind.push(hand);
                    };
            ...
            case 2: //If there are 2 unique cards, hand type = 'Full house' or 'Four of a kind'.
                ...
                for (y=0; y<2; y++) { //Loop over every card in the unique set.
                    if (handInfo.filter(x => x === uniqueCards[y]).length === 4) { //If a card appears 4 times in the full hand set, hand type = 'Four of a kind'.
                        if (jokerAmount === 0) {
                            ...
                            fourOfaKind.push(hand); 
                            break;
                        } else if (jokerAmount === 1 || jokerAmount === 4) {
                ...
                if (handType === 'Full house') {
                    if (jokerAmount === 0) { //If there is no joker, hand type remains 'Full house'.
                         fullHouse.push(hand);
                    } else if (jokerAmount === 2 || jokerAmount === 3) { //If there are 2 or 3 jokers, hand type = 'Five of a kind'.
                        handType = 'Five of a kind';
                        fiveOfAKind.push(hand);
                    };
                };
        ...
        for (let i=0; i<array.length; i++) {
            ...
            switch (array[i].charAt(charIndex)) {
                case "T":
                    cardValue = 10;
                    break;
                case "J": //Joker has the lowest value of "1".
                    cardValue = 1;
                    break;
                case "Q": 
                    cardValue = 12;
    ...
    function reverseCardValues (value) {
        ...
        switch (value) {
            case 1: //Joker has the lowest value of "1".
                value = "J";
                break;
            case 10:
                value = "T";
